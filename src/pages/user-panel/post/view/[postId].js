...
import { getUserPostDetailSEOAPI } from '@/api/apiAuthService';
import { useDispatch } from "react-redux";
import { setHeadSeo_ACTION } from "@/store/headSeoSlice";
// Component
import SEO from '@/components/SEO';
const UserPostView = dynamic(() => import('@/components/UserPostView'), {
    ssr: false,
    loading: () => null
});
...
export async function getStaticProps() {
    try {
        const seoData = await getUserPostDetailSEOAPI();
        return {
            props: { seoData }
        };
    } catch (error) {
...
export default function UserPostViewPage({ seoData }) {
    ...
    return (
        <>
            ...
            <UserPostView /> {/* Lazy-loaded on client-side */}
        </>
    );
}